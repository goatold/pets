...
// add ending "/r/n" to sdp
  if (strcasestr(msg, "application/sdp")) {
    size_t ml;
    ml = 0;
    ml = strlen(msg);
    if ( msg[ml-2] != '\r' || msg[ml-1] != '\n' ) {
      char msgb[strlen(msg)+3];
      strcpy(msgb, msg);
      strcat(msgb, "\r\n");
      rc = write_socket(sock, msgb, strlen(msgb), WS_BUFFER, &call_peer);
    } else {
